...
def create_payment_order(token: str, target_type: str, target_id: str) -> Optional[dict]:
    ...
    if target_type == "task":
        resp = post(f"/api/v1/payments/task/{target_id}/order", 
                   {"idempotencyKey": idempotency_key}, 
                   headers=headers)
    elif target_type == "batch":
        resp = post(f"/api/v1/payments/batch/{target_id}/order", 
                   {"idempotencyKey": idempotency_key}, 
                   headers=headers)
    else:
        return None
...
def get_payment_status(token: str, target_type: str, target_id: str) -> Optional[dict]:
    ...
    if target_type == "task":
        resp = get(f"/api/v1/payments/task/{target_id}", headers=headers)
    elif target_type == "batch":
        resp = get(f"/api/v1/payments/batch/{target_id}", headers=headers)
    else:
        return None
...
def test_1_instant_booking_with_payment():
    ...
    if resp.status_code == 200:
        ...
        p("Task status is POSTED", task.get("status") == "POSTED")
        ...
        p("Payment status endpoint accessible", payment_status is not None or resp.status_code == 404)
...
def test_2_bulk_booking_small_crew():
    ...
    p("Bulk booking preview works", resp.status_code == 200, f"status={resp.status_code}")
    perf("Bulk preview", response_time, 1500)
    
    if resp.status_code == 200:
        preview = resp.json()
        p("Preview has cost breakdown", "totalCostPaise" in preview or "estimatedCost" in preview)
    
    # Step 2: Create bulk booking
    resp = post("/api/v1/tasks/bulk", bulk_data, headers=headers)
    p("Create bulk booking with 3 helpers", resp.status_code == 200, f"status={resp.status_code}")
    ...
    if resp.status_code == 200:
        ...
        if resp.status_code == 200:
            summary = resp.json()
            p("Batch summary has required fields", all(k in summary for k in ["title", "status", "items"]))
...
def test_3_bulk_booking_large_crew():
    ...
    large_bulk = {
        ...
        "notes": "Need 12 helpers for event setup",
        "requestedHelperCount": 12,
        "scheduledAt": scheduled_time,
        "items": [
            {
                "title": f"Event setup task {i}",
                "description": "Help with event arrangements",
                "timeMinutes": 180,
                "budgetPaise": 80000,
                "lat": HYD_LAT,
                "lng": HYD_LNG,
                "addressText": f"Event venue section {i}"
            } for i in range(1, 4)
        ]
    }
    ...
    instant_large.pop("scheduledAt", None)
    ...
    if resp.status_code == 200:
        ...
        # Step 3: Check batch status (should be PENDING_MEDIATOR)
        resp = get(f"/api/v1/batches/{batch_id}/summary", headers=headers)
        if resp.status_code == 200:
            summary = resp.json()
            p("Large batch has PENDING_MEDIATOR status", "MEDIATOR" in summary.get("status", ""))
...
def test_4_schedule_later_booking():
    ...
    if resp.status_code == 200:
        ...
        new_scheduled_time = (datetime.now(timezone.utc) + timedelta(hours=4)).isoformat()
        resp = put(f"/api/v1/tasks/{task_id}", {"scheduledAt": new_scheduled_time}, headers=headers)
        p("Can reschedule task", resp.status_code in [200, 404], f"status={resp.status_code}")
...
def test_8_security_vulnerabilities():
    ...
    for payload in sql_injection_payloads:
        ...
        p(f"SQL injection blocked ({payload[:20]}...)", resp.status_code in [400, 422], f"status={resp.status_code}")
...
def test_12_admin_panel_features():
    ...
    if resp.status_code == 200:
        summary = resp.json()
        has_metrics = any(k in summary for k in ["activeTasks", "totalBuyers", "totalHelpers", "totalRevenue"])
